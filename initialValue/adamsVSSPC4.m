function [ t, yt ] = adamsVSSPC4( fty, lowerLimit, upperLimit, initialCondition, tolerance, stepmin, stepmax )
%ADAMSVSSPC4 Estimate value of the initial-value problem using the Adams
%               Variable Step-Size Predictor-Corrector Method. Uses the
%               Adams-Bashforth 4-step Method and the Adams-Moulton 3-step
...
% use Runge-Kutta Order 4 to get next 3 values
[t, yt] = subrk4(fty, step, i, i + 3, lowerLimit, initialCondition);
NFLAG = 1;
...
while FLAG && abs(step) ~= 0
    % Adams-Bashforth 4-step Method to get an estimate
    wp = yt(i - 1) + step / 24 * ( 55 * feval(fty, t(i - 1), yt(i - 1)) - 59 * feval(fty, t(i - 2), yt(i - 2)) + 37 * feval(fty, t(i - 3), yt(i - 3)) - 9 * feval(fty, t(i - 4), yt(i - 4)) );
    % Adams-Moulton 3-step Method to correct the estimate
    wc = yt(i - 1) + step / 24 * ( 9 * feval(fty, tt, wp) + 19 * feval(fty, t(i - 1), yt(i - 1)) - 5 * feval(fty, t(i - 2), yt(i - 2)) + 1 * feval(fty, t(i - 3), yt(i - 3)) );
    % estimate error
    ...
    if alpha <= tolerance % if the estimate is within the tolerance
        ...
        if LAST % if we calculated the last value
            ...
            if alpha <= 0.1 * tolerance || t(i - 1) + step > upperLimit
                ...
                if t(i - 1) + 4 * step > upperLimit % if we're at the end
                    % make sure step size does not go out of bounds
                    step = ( upperLimit - t(i - 1) ) / 4;
                    LAST = 1;
                end
                % calculate next Runge-Kutta values
                [t, yt] = subrk4(fty, step, i - 1, i + 2, t, yt);
                NFLAG = 1; % we have not accepted these values yet
                i = i + 3; % go one past all calculated values
            end
        end
